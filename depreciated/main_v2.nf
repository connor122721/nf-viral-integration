    ...
    if (params.samples || params.patient_dir || params.patient_bam || params.patient_fastq) {
        ...
        if (params.samples) {
                ...
                .map { row ->
                    ...
                    if (!hasVal(row, 'fastq') && !hasVal(row, 'bam')) {
                        throw new IllegalArgumentException(
                            "Samplesheet row '${row.sample_id ?: '<no sample column>'}' has neither " +
                            "a 'fastq' nor a 'bam' value. Populate one of them.")
                    }
                    row
                }

            // ---- FASTQ rows: always pass straight through ----------------------
            // Independent of demux settings — lima only ever operates on BAMs.
            fastq_rows_ch = samplesheet_ch
                .filter { row -> hasVal(row, 'fastq') }
                .map    { row -> tuple(row.sample_id, file(row.fastq, checkIfExists: true)) }

            // ---- BAM rows: only rows with a non-empty 'bam' cell ----------------
            // If the samplesheet is FASTQ-only this channel is simply empty and
            // DEMUX / RENAME_DEMUXED_BAM / BAM_TO_FASTQ spawn zero tasks.
            bam_rows_ch = samplesheet_ch.filter { row -> hasVal(row, 'bam') }
            ...
            if (!params.skip_demux) {
                ...
                demux_input_rows = bam_rows_ch.map { row ->
                ...
                // skip_demux: treat every BAM row as already demultiplexed
                bam_files_ch = bam_rows_ch.map { row -> file(row.bam, checkIfExists: true) }
            }
