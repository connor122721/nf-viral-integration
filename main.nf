...
workflow {
    ...
    // POST_PROCESS expects per-sample integrations as tuple(sample_id, tsv).
    // INTEGRATION_ANNOTATE.out.csv already carries (sample_id, csv). We accept
    // either CSV or TSV; the downstream Rscripts use data.table::fread which
    // sniffs the separator. Re-publish as .tsv if you prefer strict typing.
    integrations_per_sample_ch = INTEGRATION_ANNOTATE.out.csv
        .map { sample_id, csv -> tuple(sample_id, csv) }
