...
########################################################
# inFN      <- "m84248_240819_203719_s2.hifi_reads.univ_v3_bc1003.unmasked.fa"
# OUT_FN    <- "m84248_240819_203719_s2.hifi_reads.univ_v3_bc1003.viral.tsv"
# reference <- "HIV_HXB2"   # replace with your actual reference name
# virusType <- "HIV"
# PREFIX    <- "/home/c0murr09/nf-viral-integration_t2t"
  ...
  best <- tibble(gene = genes) %>%
    mutate(data = map(gene, function(g) {
      ...
      tibble(beg = iv$beg, end = iv$end,
             len = abs(iv$end - iv$beg) + 1L)  # gene carried by outer tibble
    })) %>%
    unnest(data)
...
update_match_string <- function(s, beg_flag, end_flag) {
  if (beg_flag) {
    matched <- str_extract(s, "^0+")
    n <- if (is.na(matched)) 0L else str_length(matched)
    if (n > 0) s <- str_replace(s, "^0+", str_c(rep("-", n), collapse = ""))
  }
  if (end_flag) {
    matched <- str_extract(s, "0+$")
    n <- if (is.na(matched)) 0L else str_length(matched)
...
########################################################
## APPLY MATCH STRING POST-CORRECTIONS 
...
correct_match_string <- function(match_str, t5, t3) {
  ...
  # [123]0000000[123]  (gene at both ends, nothing in between)
...
write_tsv(results_tbl, paste0(OUT_FN, ".txt"), na = "")
write_csv(results_tbl, paste0(OUT_FN, ".csv"), na = "")
...
walk(c("INTACT","PUTATIVELY INTACT","INDETERMINATE",
     ...
     ~cat(.x, ":", ifelse(is.na(type_counts[.x]), 0L, type_counts[.x]), "\n"))
...
nonzero_pos <- which(ltr_pos_vec > 0)
cat("MIN LTR LENGTH:", if (length(nonzero_pos) > 0) min(nonzero_pos) else 0L, "\n")
