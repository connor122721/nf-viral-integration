...
package_names <- package_names[!package_names =="tcltk"]
...
lapply(package_names, library, character.only = TRUE)
  ...
  tolerance <- 10           ## Tolerance to make iden#tical calls in the context of indels
 ...
 getReadsFromCSVFiles <- function() {
  ...
  cleanReads <- function(reads, tolerance, eFlag) {
    ##tic("cleanReads")
     ...
     #print(Cleaned.reads$fivePrimeGap[Cleaned.reads$Flanking == "Dually-Flanked"])
  ...
  standardizeCloneIDs <- function(validIntermediateCount) {
    ...
    cat("Addressing overlapping but non-iden##tical ranges.\n")
  ...
  markClonalExpansion <- function(Cleaned.reads) {
    ...
    vBF <- getClones(validReadsDualFlanked, "Dually-Flanked") #THESE ARE BEING PROBLEMA#tic
    validIntermediateCount <- vBF 
  ...
  removeDuplicatedColumns <- function(dataframe) {
    ...
    cat("checking problema#tic columns.\n")
