...
process INTEGRATION_ANNOTATE {
    ...
    script:
        ...
        # 3. PROPER MERGE (replaces shell `join` + manual header) -----------
        # Preserves column NAMES and order so downstream scripts can address
        # columns by name without drift.
        ...
        # 4. Circos PNG (per-sample) — must exist before the HTML script ----
        # Rscript ${projectDir}/bin/make_circos.R \\
        #    --integrations ${sample_id_i}.combined.csv \\
        #    --mode sample \\
        #    --genome ${report_genome} \\
        #    --label "${sample_id_i}" \\
        #    --out ${sample_id_i}_circos.png 

        # 5. Sample HTML report ---------------------------------------------
        # Rscript ${sample_report_script} \\
        #    --integrations ${sample_id_i}.combined.csv \\
        #    --sample_id    ${sample_id_i} \\
        #    --circos_png   ${sample_id_i}_circos.png
        """
}
